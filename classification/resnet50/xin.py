...
from fenlei_vmunet import VMUNet
...
def train_and_valid(model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):
        ...
        if (epoch > 290):
            torch.save(model.state_dict(), '/disk/sdb/zhangqian0620/fenlei/save_mamba/'  + str(epoch + 1) + '.pth')
...
if __name__ == '__main__':
   ...
   #对数据进行一个封装
    ...
    if torch.cuda.is_available():
        mamba_fenlei = mamba_fenlei.to('cuda:0')
        print("*" * 20, "使用了GPU！")
    else:
        print("*" * 20, "使用了CPU！")
    # 定义损失函数和优化器。
   # loss_func = nn.NLLLoss()
    ...
    #loss_func=net.Loss()
    optimizer = optim.Adam(mamba_fenlei.parameters())

   # optimizer = optim.SGD(resnet50.parameters())
    num_epochs =300
    #跳转到train_and_valid方法
    trained_model, history = train_and_valid(mamba_fenlei, loss_func, optimizer, num_epochs)
    print(trained_model)
    torch.save(history, 'models_mamba/' + '_history.pt')
